...
#include "SusyAnaTools/TopTagger/interface/TopCandidate.h"
...
namespace topTagger
{
    ...
    public:
        ...
        // DON'T OVERRIDE THEM!
        bool isTopEvent;
        vector<vector<int> > finalCombfatJets;
        vector<vector<int> > finalRemaining;
        vector<vector<vector<int> > > finalCombJetSubStruc;
        vector<double> fatJetMassVec;
        vector<vector<double> > fatJetSubMassVec;
        ...
        int pickedbTaggedJetIdx;
        double maxCSVS;
        bool bestPassCSVS;
        vector<int> pickedbTaggedbestTopJetIdxVec;

        vector<vector<int> > finalRemainingCombfatJets;
        vector<vector<int> > finalRemaining2;
        vector<vector<vector<int> > > finalRemainingCombJetSubStruc;
        vector<double> finalRemainingCombMaxDR;
        int pickedRemainingCombfatJetIdx;
        double pickedRemainingCombfatJetMass;
        int pickedRemainingCombfatJetbJetIdx;
        int cntbTaggedIndex;

        vector<int> pickedRemainingCombfatJet;
        vector<vector<int> > pickedRemainingCombJetSubStruc;
        ...
        int pickedClosestJetTobIdx;
        double pickedClosestJetTobDeltaR;
        ...
        int closebJetIdx;
        double closebJetDphi;
        TLorentzVector closebJetLVec;
        double mTbJet;

        double MT2;
        // For HepTopTagger
        std::vector<TLorentzVector> modiforiJetsVec;
        std::vector<double> modifrecoJetsBtagCSVS;
        ...
        void setDefaultVars ()
        {
            ...
            finalCombfatJets.clear ();
            finalRemaining.clear ();
            finalCombJetSubStruc.clear ();
            fatJetMassVec.clear ();
            fatJetSubMassVec.clear ();
            finalCombMaxDR.clear ();
            cntnbJetsCSVS = 0;
            combSize = -1;
            finalCombfatJetsPassStatusVec.clear ();
            cntTopCandPassCSVSVec.clear ();
            cntRemainingPassCSVSVec.clear ();
            nTopJetCandCnt = 0;
            nTopJetCandWithBtagCnt = 0;
            bestTopJetIdx = -1;

            allCombPassCriteriaVec.clear ();

            bestTopJetMass = -1;
            bestTopJetComb.clear ();
            bestTopJetSubStruc.clear ();
            ...
            remainPassCSVS = false;
            pickedbTaggedJetIdxVec.clear ();
            pickedbTaggedJetIdx = -1;
            maxCSVS = -1;
            bestPassCSVS = false;
            pickedbTaggedbestTopJetIdxVec.clear ();

            finalRemainingCombfatJets.clear ();
            finalRemaining2.clear ();
            finalRemainingCombJetSubStruc.clear ();
            finalRemainingCombMaxDR.clear ();
            pickedRemainingCombfatJetIdx = -1;
            pickedRemainingCombfatJetMass = -1;
            pickedRemainingCombfatJetbJetIdx = -1;
            cntbTaggedIndex = -1;

            pickedRemainingCombfatJet.clear ();
            pickedRemainingCombJetSubStruc.clear ();

            pickedRemainingfatJetMass = -1;
            pickedRemainingSubMassVec.clear ();
            ...
            pickedClosestJetTobIdx = -1;
            pickedClosestJetTobDeltaR = -1;
            ...
            modiforiJetsVec.clear ();
            modifrecoJetsBtagCSVS.clear ();
        ...
        void buildSubStrucIndexComb (vector<int> indexVec, vector<vector<int> > &tmpFinalCombSubStruc)
        {
            ...
            if (indexVec.size () <= 2)
            {
                tmpFinalCombSubStruc.push_back (indexVec);
                return;
            }

            vector<int> iniIndexVec;
            iniIndexVec.push_back (indexVec[0]);
            iniIndexVec.push_back (indexVec[1]);
        ...
        void prepareCombination (const vector<int> & indexVec, vector<vector<int> > &tmpFinalCombfatJets, vector<vector<int> > &tmpFinalRemaining, vector<vector<vector<int> > > &tmpFinalCombJetSubStruc)
        {
            ...
            if (nSubJetsDiv_ > (int) indexVec.size ())
            {
                std::cout << "indexVec.size : " << indexVec.size () << " is SHORTER than nSubJetsDiv_ : " << nSubJetsDiv_ << "?!" << std::endl;
                return;
            }

            vector<int> iniIndexComb;
            for (int ij = 0; ij < nSubJetsDiv_; ij++) iniIndexComb.push_back (indexVec[ij]);
            ...
            for (unsigned int ic = 0; ic < tmpCombSize; ic++)
            {
                ...
                buildSubStrucIndexComb (tmpFinalCombfatJets[ic], perJetSub);
                tmpFinalCombJetSubStruc.push_back (perJetSub);
        ...
        void makeWjetfatJets (const vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS, vector<vector<int> > &tmpFinalCombfatJets, vector<vector<int> > &tmpFinalRemaining, vector<vector<vector<int> > > &tmpFinalCombJetSubStruc)
        {
            ...
            for (unsigned int iw = 0; iw < tmpWjetIdx.size (); iw++)
            {
                for (unsigned int ir = 0; ir < tmpRemainIdx.size (); ir++)
                {
                    ...
                    perComb.push_back (tmpWjetIdx[iw]);
                    perComb.push_back (tmpRemainIdx[ir]);
        ...
        void onlyfatJetsCloseInACone (const vector<int> &indexVec, const vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS, vector<vector<int> > &tmpFinalCombfatJets, vector<vector<int> > &tmpFinalRemaining, vector<vector<vector<int> > > &tmpFinalCombJetSubStruc)
        {
            ...
            vector<vector<int> > tempCombfatJets;
            vector<vector<int> > tempRemaining;
            vector<vector<vector<int> > > tempCombJetSubStruc;
        ...
        void prepareFindingBestTopCandidate (const std::vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS)
        {
            ...
            vector<int> indexVec;
            for (size ij = 0; ij < oriJetsVec.size (); ij++) indexVec.push_back (ij);
        ...
        void findBestTopCandidate (const std::vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS)
        {
            ...
            for (int ic = 0; ic < combSize; ic++)
            {
                ...
                if (doInvertTopCriteria_)
                {
                    if (isTopJet) isTopJet = 0;
                    else isTopJet = 1;
                }
        ...
        void runOnlyTopTaggerPart (const std::vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS)
        {
            ...
            CSVS_ = minCSVS_disablebtag;
            globalCachedCSVS_ = minCSVS_disablebtag;
            maxEtaForbJets_ = maxEta_disablebtag;
            bVetoMethod_ = 1;
            ...
            CSVS_ = cachedCSVS;
            globalCachedCSVS_ = cachedglobalCachedCSVS;
            maxEtaForbJets_ = cachedmaxEtaForbJets;
            bVetoMethod_ = cachedbVetoMethod;
        ...
        int findnTopCands (const std::vector<TLorentzVector> &oriJetsVec)
        {
            ...
            for (mapIter = topJetCand_deltaM_idx123Map.begin (); mapIter != topJetCand_deltaM_idx123Map.end (); mapIter++)
            {
                ...
                if (minDeltaM != -1 && minDeltaM >= mapIter->first)
                {
                    std::cout << "WARNING ... Type3TopTagger ... map not sorted correctly as what I want?!" << std::endl;
                }
                ...
                if (!isFound)
                {
                    ...
                    for (int ic = 0; ic < combSize; ic++)
                    {
                        if (finalCombfatJets[ic] == mapIter->second) pickedCombIdx = ic;
                    }
                    if (pickedCombIdx == -1)
                    {
                        std::cout << "WARNING ... Type3TopTagger ... mis-matching between topJetCand_deltaM_idx123Map and finalCombfatJets?!" << std::endl;
                    }
        ...
        void resolveOverlappingJets (const std::vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS, const std::vector<TLorentzVector> &hepTaggerSubJetsVec, std::vector<int> &tmpremainingJetsVec, std::vector<double> &tmpremainingJetsBtagCSVS, std::vector<double> &hepTaggerSubJetsBtagCSVS)
        {
            ...
            tmpremainingJetsVec.clear ();
            tmpremainingJetsBtagCSVS.clear ();
            hepTaggerSubJetsBtagCSVS.clear ();
        ...
        void prepareANDfindBestTopCandidate (const std::vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS, const std::vector<std::vector<TLorentzVector> > &hepTaggerJetsVec, std::vector<TLorentzVector> &tmpModiforiJetsVec, std::vector<double> &tmpModifrecoJetsBtagCSVS)
        {
            ...
            tmpModiforiJetsVec.clear ();
            tmpModifrecoJetsBtagCSVS.clear ();
            ...
            for (int ic = 0; ic < combSize; ic++)
            {
                ...
                perfinalCombJetSubStruc[0].push_back (0);
                perfinalCombJetSubStruc[0].push_back (1);
                perfinalCombJetSubStruc[1].push_back (0);
                perfinalCombJetSubStruc[1].push_back (2);
                perfinalCombJetSubStruc[2].push_back (1);
                perfinalCombJetSubStruc[2].push_back (2);
                ...
                perfatJetSubMassVec.push_back (fatJetm12);
                perfatJetSubMassVec.push_back (fatJetm13);
                perfatJetSubMassVec.push_back (fatJetm23);
                fatJetSubMassVec.push_back (perfatJetSubMassVec);
            ...
            for (unsigned int ij = 0; ij < oriJetsVec.size () - 1; ij++)
            {
                ...
                for (unsigned int jj = ij + 1; jj < oriJetsVec.size (); jj++)
                {
                    ...
                    for (unsigned int kj = 0; kj < oriJetsVec.size (); kj++)
                    {
                        ...
                        if (cntnbJetsCSVS >= 2)
                        {
                            if (!(recoJetsBtagCSVS[kj] > CSVS_ && fabs (oriJetsVec[kj].Eta ()) < maxEtaForbJets_)) continue;
                        }
                        ...
                        {
                            minchisq = chisq;
                            pickedTopIdxVec.push_back (ij);
                            pickedTopIdxVec.push_back (jj);
                            pickedTopIdxVec.push_back (kj);
                        }
                        else if (minchisq > chisq)
                        {
                            minchisq = chisq;
                            pickedTopIdxVec[0] = ij;
                            pickedTopIdxVec[1] = jj;
                            pickedTopIdxVec[2] = kj;
                        }
                    }
                }
            }
        ...
        void setdoExtraCuts (const bool doExtraCuts)
        {
            doExtraCuts_ = doExtraCuts;
        }

        void setdoInvertTopCriteria (const bool doInvertTopCriteria)
        {
            doInvertTopCriteria_ = doInvertTopCriteria;
        }

        void setmTbcut (const double mTbcut)
        {
            mTbcut_ = mTbcut;
        }

        void setmTtcut (const double mTtcut)
        {
            mTtcut_ = mTtcut;
        }

        void setMT2cut (const double MT2cut)
        {
            MT2cut_ = MT2cut;
        }

        void setmTWcut (const double mTWcut)
        {
            mTWcut_ = mTWcut;
        }

        void setdoMTMT2cuts (const bool doMTMT2cuts)
        {
            doMTMT2cuts_ = doMTMT2cuts;
        }
        ...
        void setlowRatioWoverTop (const double lowRatioWoverTop)
        {
            lowRatioWoverTop_ = lowRatioWoverTop;
            if (mWoverTop_ != -1) Rmin_ = lowRatioWoverTop_ * mWoverTop_;
        }

        void sethighRatioWoverTop (const double highRatioWoverTop)
        {
            highRatioWoverTop_ = highRatioWoverTop;
            if (mWoverTop_ != -1) Rmax_ = highRatioWoverTop_ * mWoverTop_;
        }

        void setlowArcTanm13overm12 (const double lowArcTanm13overm12)
        {
            lowArcTanm13overm12_ = lowArcTanm13overm12;
        }

        void sethighArcTanm13overm12 (const double highArcTanm13overm12)
        {
            highArcTanm13overm12_ = highArcTanm13overm12;
        }

        void setlowm23overm123 (const double lowm23overm123)
        {
            lowm23overm123_ = lowm23overm123;
        }

        void setdefaultJetCone (const double defaultJetCone)
        {
            defaultJetCone_ = defaultJetCone;
        }

        void setsimuCAdeltaR (const double simuCAdeltaR)
        {
            simuCAdeltaR_ = simuCAdeltaR;
        }

        void setsimuCALargerdeltaR (const double simuCALargerdeltaR)
        {
            simuCALargerdeltaR_ = simuCALargerdeltaR;
        }

        void setlowTopCut (const double lowTopCut)
        {
            lowTopCut_ = lowTopCut;
        }

        void sethighTopCut (const double highTopCut)
        {
            highTopCut_ = highTopCut;
        }

        void setlowWCut (const double lowWCut)
        {
            lowWCut_ = lowWCut;
        }

        void sethighWCut (const double highWCut)
        {
            highWCut_ = highWCut;
        }

        void setCSVS (const double CSVS)
        {
            CSVS_ = CSVS;
            globalCachedCSVS_ = CSVS_;
            oriCSVScut_ = CSVS_;
        }

        void setnSubJetsDiv (const int nSubJetsDiv)
        {
            nSubJetsDiv_ = nSubJetsDiv;
        }

        void setnJetsSel (const int nJetsSel)
        {
            nJetsSel_ = nJetsSel;
        }

        void setmaxEtaForbJets (const double maxEtaForbJets)
        {
            maxEtaForbJets_ = maxEtaForbJets;
            orimaxEtaForbJets_ = maxEtaForbJets_;
        }

        void setorderingOptArr (const std::vector<std::string> & orderingOptArr)
        {
            orderingOptArr_ = orderingOptArr;
        }

        void setorderingOptArr ()
        {
            std::vector<std::string> orderingOptArr (defaultOrderingOptArr, defaultOrderingOptArr + sizeof (defaultOrderingOptArr) / sizeof (defaultOrderingOptArr[0]));
            orderingOptArr_ = orderingOptArr;
        }

        void setmaxIndexForOrderingArr (const std::vector<int> & maxIndexForOrderingArr)
        {
            maxIndexForOrderingArr_ = maxIndexForOrderingArr;
        }

        void setmaxIndexForOrderingArr ()
        {
            std::vector<int> maxIndexForOrderingArr (defaultMaxIndexForOrderingArr, defaultMaxIndexForOrderingArr + sizeof (defaultMaxIndexForOrderingArr) / sizeof (defaultMaxIndexForOrderingArr[0]));
            maxIndexForOrderingArr_ = maxIndexForOrderingArr;
        }

        void setmaxDRjetMatch (const double maxDRjetMatch)
        {
            maxDRjetMatch_ = maxDRjetMatch;
        }
        ...
        void settaggingMode (const bool taggingMode)
        {
            taggingMode_ = taggingMode;
        }

        void setdobVetoCS (const bool dobVetoCS)
        {
            dobVetoCS_ = dobVetoCS;
            globalCacheddobVetoCS_ = dobVetoCS_;
            if (dobVetoCS_)
            {
                CSVS_ = minCSVS_disablebtag;
                globalCachedCSVS_ = CSVS_;
        ...
        void setbVetoMethod (const int bVetoMethod)
        {
            bVetoMethod_ = bVetoMethod;
        }

        void setWjetMassRanges (const double lowWjetMass, const double highWjetMass)
        {
            lowWjetMass_ = lowWjetMass;
            highWjetMass_ = highWjetMass;
        }

        void setdoWjetfatJets (const bool doWjetfatJets)
        {
            doWjetfatJets_ = doWjetfatJets;
        }

        void setTopjetMassRanges (const double lowTopjetMass, const double highTopjetMass)
        {
            lowTopjetMass_ = lowTopjetMass;
            highTopjetMass_ = highTopjetMass;
        }

        void setdoTopjetfatJets (const bool doTopjetfatJets)
        {
            doTopjetfatJets_ = doTopjetfatJets;
        }

        void setdoCombTopTagger (const bool doCombTopTagger)
        {
            doCombTopTagger_ = doCombTopTagger;
        }

        void setlowTopPtToComb (const double lowTopPtToComb)
        {
            lowTopPtToComb_ = lowTopPtToComb;
        }

        void setgroomedJetsVec (const std::vector<TLorentzVector> groomedJetsVec)
        {
            groomedJetsVec_ = groomedJetsVec;
        }

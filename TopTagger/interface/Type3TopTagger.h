...
namespace topTagger{
   ...
   typedef vector<vector<vector<int> > > subStructVec;
...
// DON'T OVERRIDE THEM!
         bool isTopEvent;
         vector<vector<int> > finalCombfatJets; vector<vector<int> > finalRemaining; subStructVec finalCombJetSubStruc;
         ...
         vector<vector<int> > finalRemainingCombfatJets; vector<vector<int> > finalRemaining2; subStructVec finalRemainingCombJetSubStruc;
...
*/
         ...
         type3TopTagger(){
            dt1 = dt2 = dt3 = dt4 = dt5 = dt6 = dt7 = dt8 = dt9 = 0.0;
         ...
         void buildOneFatJetsIndexComb(const std::vector<TLorentzVector> &oriJetsVec, vector<int> indexVec, vector<int> &iniIndexComb, vector<vector<int> > &tmpFinalCombJets, vector<vector<int> > &tmpFinalRemaining)
         {
         
             tmpFinalCombJets.clear();
             tmpFinalRemaining.clear();
         
             if(iniIndexComb.size() == 3)
             {
                 //const unsigned int N = indexVec.size()*(indexVec.size() - 1)*(indexVec.size() - 2)/6;
         
                 //tmpFinalCombJets.reserve(N);
                 //tmpFinalRemaining.reserve(N);
         
                 for(unsigned int i = 0; i < indexVec.size(); ++i)
                 {
                     for(unsigned int j = i + 1; j < indexVec.size(); ++j)
                     {
                         for(unsigned int k = j + 1; k < indexVec.size(); ++k)
                         {
                             vector<int> fatJetIdxs({indexVec[i], indexVec[j], indexVec[k]});
                             if(filterfatJetWithDRcone(oriJetsVec, fatJetIdxs, simuCAdeltaR_) )
                             {
                                 tmpFinalCombJets.emplace_back(fatJetIdxs);
                                 tmpFinalRemaining.emplace_back(vector<int>());
                                 tmpFinalRemaining.back().reserve(indexVec.size() - iniIndexComb.size());
                                 for(const int& index : indexVec) if(index != indexVec[i] && index != indexVec[j] && index != indexVec[k]) tmpFinalRemaining.back().emplace_back(index);
                             }
                         }
                     }
                 }
                 ...
                 //{
                 //    tmpFinalCombJets.emplace_back(vector<int>({indexVec[i], indexVec[j], indexVec[k]}));
                 //    tmpFinalRemaining.emplace_back(vector<int>());
                 //    tmpFinalRemaining.back().reserve(indexVec.size() - iniIndexComb.size());
                 //    for(const int& index : indexVec) if(index != indexVec[i] && index != indexVec[j] && index != indexVec[k]) tmpFinalRemaining.back().emplace_back(index);
                 //    
                 //    //nextCombInd(i, j, k, indexVec.size());
                 //    if(k == indexVec.size() - 1)
                 //    {
                 //        if(j == k - 1)                     
                 //        {
                 //            ++i;
             ...
             do
             {
                 if( filterfatJetWithDRcone(oriJetsVec, iniIndexComb, simuCAdeltaR_) )
                 {
                     tmpFinalCombJets.push_back(iniIndexComb);
                     tmpFinalRemaining.push_back(vector<int>());
                     vector<int>::const_iterator it = iniIndexComb.begin();
                     for(const int i : indexVec)
                     {
                         if(it == iniIndexComb.end() || i != *it) tmpFinalRemaining.back().push_back(i);
                         else ++it;
                     }
                 }
             }
         ...
         //      }
         //      //for(int i : tmpFinalCombJets.back()) std::cout << i << "\t";
         //      //std::cout << "|\t";
         //      //for(int i : tmpFinalRemaining.back()) std::cout << i << "\t";
         //      //cout << endl;
         //   }
         //}

         void buildSubStrucIndexComb(vector<int>& indexVec, subStructVec &finalCombSubStruc)
         {
             if(indexVec.size() <= 2)
             {
                 finalCombSubStruc.push_back({indexVec});
                 return;
             }
             else if(indexVec.size() == 3)
             {
                 finalCombSubStruc.emplace_back(vector<vector<int>>({{indexVec[0], indexVec[1]}, {indexVec[0], indexVec[2]}, {indexVec[1], indexVec[2]}}));
                 return;
             }

             finalCombSubStruc.push_back(vector<vector<int>>());
             ...
             do
             {
                 finalCombSubStruc.back().push_back(iniIndexVec);
             }
         ...
         TLorentzVector buildLVec(const vector<TLorentzVector> &oriJetsVec, const vector<int> &indexCombVec)
         {
             if(indexCombVec.size() == 3) return oriJetsVec[indexCombVec[0]] + oriJetsVec[indexCombVec[1]] + oriJetsVec[indexCombVec[2]];

             TLorentzVector combLVec;
             for(const int& index : indexCombVec)
             {
                 combLVec += oriJetsVec[index];
             }
             return combLVec;
         }
         ...
         bool filterfatJetWithDRcone(const vector<TLorentzVector> &oriJetsVec, const vector<int> &indexCombVec, const double drCut)
         {
            ...
            for(const int& index : indexCombVec)
            {
               double deltaR = centralLVec.DeltaR(oriJetsVec[index]);
               if( deltaR > drCut ) return false;
            }
         ...
         void prepareCombination(const std::vector<TLorentzVector> &oriJetsVec, const vector<int> & indexVec, vector<vector<int> > &tmpFinalCombfatJets, vector<vector<int> > &tmpFinalRemaining, subStructVec &tmpFinalCombJetSubStruc){
            ...
            buildOneFatJetsIndexComb(oriJetsVec, indexVec, iniIndexComb, tmpFinalCombfatJets, tmpFinalRemaining);

            t2 = std::chrono::system_clock::now();

            tmpFinalCombJetSubStruc.reserve(tmpFinalCombfatJets.size());
            for(vector<int>& combfatJet : tmpFinalCombfatJets)
            {
                buildSubStrucIndexComb(combfatJet, tmpFinalCombJetSubStruc);
            }

            t3 = std::chrono::system_clock::now();

            std::chrono::duration<double> dt1l = t1 - t0;
            std::chrono::duration<double> dt2l = t2 - t1;
            std::chrono::duration<double> dt3l = t3 - t2;
            dt4 += dt1l.count();
            dt5 += dt2l.count();
            dt6 += dt3l.count();
         }

         void ptOrderedfatJets(const vector<TLorentzVector> &oriJetsVec, vector<vector<int> > &tmpFinalCombfatJets, vector<vector<int> > &tmpFinalRemaining, subStructVec &tmpFinalCombJetSubStruc){
            ...
            const subStructVec cachedtmpFinalCombJetSubStruc = tmpFinalCombJetSubStruc;
...
// Transfer the fat jets to pt ordered
         ...
         void makeWjetfatJets(const vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS, vector<vector<int> > &tmpFinalCombfatJets, vector<vector<int> > &tmpFinalRemaining, subStructVec &tmpFinalCombJetSubStruc)
         {
             ...
             const std::vector<TLorentzVector> *forJetMassLVec = &oriJetsVec;
             if( !groomedJetsVec_.empty() )
             {
                 cout << "Groomed Jets not implemented!!! (Still using origional jet vector)" << endl;
...
// in this case the only sub-structure is the W jet!
             for(int& wJetIdx : tmpWjetIdx)
             {
                 for(int& remainIdx : tmpRemainIdx)
                 {
                     vector<int> perComb({wJetIdx, remainIdx});
                     if( filterfatJetWithDRcone(oriJetsVec, perComb, simuCAdeltaR_) )
                     {
                         vector<int> perRemain;
                         for(int ij=0; ij<(int)oriJetsVec.size(); ij++)
                         {
                             if( ij == wJetIdx || ij == remainIdx) continue;
                             perRemain.push_back(ij);
                         }
                         tmpFinalCombfatJets.emplace_back(perComb);
                         tmpFinalRemaining.emplace_back(perRemain);
                         tmpFinalCombJetSubStruc.emplace_back(vector<vector<int> >({{wJetIdx}}));
                     }
                 }
             }
         }

         void makeTopjetfatJets(const vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS, vector<vector<int> > &tmpFinalCombfatJets, vector<vector<int> > &tmpFinalRemaining, subStructVec &tmpFinalCombJetSubStruc)
         {
             //tmpFinalCombfatJets.clear(); 
             //tmpFinalRemaining.clear(); 
             //tmpFinalCombJetSubStruc.clear();
             ...
             for(unsigned int ij=0; ij<oriJetsVec.size(); ij++)
             {
                 const TLorentzVector& perTopjetLVec = (*forJetMassLVec)[ij];
             ...
             for(int& topJetIdx : tmpTopjetIdx)
             {
                 vector<int> perRemain;
                 perRemain.reserve(oriJetsVec.size() - 1);
                 for(int ij = 0; ij < (int)oriJetsVec.size(); ij++)
                 {
                     if( ij == topJetIdx ) continue;
                     perRemain.push_back(ij);
                 }
                 tmpFinalCombfatJets.emplace_back(vector<int>({topJetIdx}));
                 tmpFinalRemaining.emplace_back(perRemain);
                 tmpFinalCombJetSubStruc.emplace_back(vector<vector<int>>());
             }
         }

         void onlyfatJetsCloseInACone(const vector<int> &indexVec, const vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS, vector<vector<int> > &tmpFinalCombfatJets, vector<vector<int> > &tmpFinalRemaining, subStructVec &tmpFinalCombJetSubStruc){

            tmpFinalCombfatJets.clear(); tmpFinalRemaining.clear(); tmpFinalCombJetSubStruc.clear();
         
            //vector<vector<int> > tempCombfatJets; vector<vector<int> > tempRemaining; subStructVec tempCombJetSubStruc;
         
            //prepareCombination(oriJetsVec, indexVec, tempCombfatJets, tempRemaining, tempCombJetSubStruc);
            prepareCombination(oriJetsVec, indexVec, tmpFinalCombfatJets, tmpFinalRemaining, tmpFinalCombJetSubStruc);

            std::chrono::time_point<std::chrono::system_clock> t0, t1, t2, t3;
            t0 = std::chrono::system_clock::now();

            //for(unsigned int ic=0; ic<tempCombfatJets.size(); ic++){
            ...
            if( doWjetfatJets_ )
            {
                ...
                //subStructVec tempWjetCombJetSubStruc;
                
                //makeWjetfatJets(oriJetsVec, recoJetsBtagCSVS, tempWjetCombfatJets, tempWjetRemaining, tempWjetCombJetSubStruc);
                makeWjetfatJets(oriJetsVec, recoJetsBtagCSVS, tmpFinalCombfatJets, tmpFinalRemaining, tmpFinalCombJetSubStruc);
            ...
            { 
                ...
                //subStructVec tempTopjetCombJetSubStruc;
               
                //makeTopjetfatJets(oriJetsVec, recoJetsBtagCSVS, tempTopjetCombfatJets, tempTopjetRemaining, tempTopjetCombJetSubStruc);
                makeTopjetfatJets(oriJetsVec, recoJetsBtagCSVS, tmpFinalCombfatJets, tmpFinalRemaining, tmpFinalCombJetSubStruc);
...
//      if( (int)bestTopJetComb.size() + remainJetNum != (int)oriJetsVec.size() ){ std::cout<<"WARNING ... bestTopJetComb.size : "<<bestTopJetComb.size()<<" + "<<"  remainJetNum : "<<remainJetNum<<"  NOT equal to oriJetsVec.size : "<<oriJetsVec.size()<<std::endl; }
         ...
         void prepareFindingRemainingTopCandidate(const std::vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS){
            ...
            prepareCombination(oriJetsVec, remainTopJetComb, finalRemainingCombfatJets, finalRemaining2, finalRemainingCombJetSubStruc);
...
// before any further processing including nJetsSel verification!
            ...
            if( noriJets >= nJetsSel_ ){
               ...
               //if(!dobVetoCS_) 
               //{
               //    printf("dt1: %10.2f  dt2: %10.2f  dt3: %10.2f  dt4: %10.2f  dt5: %10.2f  dt6: %10.2f  dt7: %10.2f  dt8: %10.2f  dt9: %10.2f", dt1*1000.0, dt2*1000.0, dt3*1000.0, dt4*1000.0, dt5*1000.0, dt6*1000.0, dt7*1000.0, dt8*1000.0, dt9*1000.0);
               //    cout << endl;
               //}

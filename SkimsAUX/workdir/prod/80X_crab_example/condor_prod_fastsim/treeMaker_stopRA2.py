...
options.register('ntpVersion', "Ntp_80X_06Jun2016_v7.0", VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.string, "ntpVersion: to be same as the tag of the release. But can be used to produce 72X ntuple as well!")
options.register('GlobalTag', "80X_mcRun2_asymptotic_2016_miniAODv2_v0", VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.string, "74X PromptReco: 74X_dataRun2_Prompt_v0")
...
options.register('jecDBname', "Spring16_25nsV3_MC", VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.string, "Summer15_25nsV6_DATA for data")
...
options.register('startfile',  0, VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.int, "start file index")
options.register('numfile',  0, VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.int, "number of files")

options.register('fastsim', True, VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.bool, "fastsim sample or not, default is False")
...
if options.fileslist:
   if os.path.exists(options.fileslist) == False or os.path.isfile(options.fileslist) == False:
      ...
      for line in ifile.readlines():
         cntIdx += 1
         if cntIdx < options.startfile:
            continue
         if cntIdx >= options.startfile + options.numfile:
            break
...
if options.files:
   ...
   process.source.fileNames = [
       'file:root://xrootd.unl.edu//store/mc/RunIISpring16MiniAODv1/TTJets_SingleLeptFromT_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/MINIAODSIM/PUSpring16_80X_mcRun2_asymptotic_2016_v3-v1/00000/0216816D-B902-E611-9E55-008CFA197D60.root'
...
tomodif_filename = "stopFlatNtuples.root"
if options.fileslist:
   tomodif_filename = "stopFlatNtuples_"+str(options.startfile/options.numfile)+".root"
print "\ntomodif_filename : %s\n"%tomodif_filename
process.TFileService = cms.Service("TFileService",
   fileName = cms.string(tomodif_filename)
)

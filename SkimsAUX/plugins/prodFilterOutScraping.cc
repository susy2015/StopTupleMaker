...
prodFilterOutScraping::prodFilterOutScraping(const edm::ParameterSet& iConfig)
{
...
prodFilterOutScraping::~prodFilterOutScraping()
{
}

bool prodFilterOutScraping::filter( edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (debugOn) {
    ...
    std::cout << "prodFilterOutScraping_debug: Run " << irun << " Event " << ievt << " Lumi Block " << ils << " Bunch Crossing " << bx << " Fraction " << fraction << " NTracks " << tkColl->size() << " Accepted " << accepted << std::endl;
  }
...
DEFINE_FWK_MODULE(prodFilterOutScraping);

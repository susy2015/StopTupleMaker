...
prodGenInfo::prodGenInfo(const edm::ParameterSet & iConfig)
{
  ...
  //2 types of neutrino TLVec 
  produces<std::vector<TLorentzVector>>("NuFromBLVec");
  produces<std::vector<TLorentzVector>>("NuOtherLVec");
}
...
bool prodGenInfo::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //add 2 types of neutrinos
  std::auto_ptr<std::vector<TLorentzVector>> NuFromBLVec( new std::vector<TLorentzVector> () );
  std::auto_ptr<std::vector<TLorentzVector>> NuOtherLVec( new std::vector<TLorentzVector> () );
  ...
  for(edm::View<reco::GenParticle>::const_iterator iPart = genParticles->begin(); iPart != genParticles->end(); ++iPart)
  {
    ...
    if(    std::find( pdgIdOfInterest.begin(), pdgIdOfInterest.end(), abs(iPart->pdgId()) ) != pdgIdOfInterest.end() 
        && ( ( abs(iPart->pdgId()) != 1000021 && abs(iPart->pdgId()) != 1000006 && abs(iPart->pdgId()) != 1000005 && abs( iPart->status() ) >20 && abs( iPart->status() ) <30 ) 
          || ( (abs(iPart->pdgId()) == 1000021 || abs(iPart->pdgId()) == 1000006 || abs(iPart->pdgId()) == 1000005) && iPart->isLastCopy() ) ) ) 
        //special requirement for gluinos || stops || sbottoms  
    {
      TLorentzVector temp;
    ...
    //add 2 types of neutrino LVec, one for from b, the other for not from b
    if( (std::abs(iPart->pdgId())==12 || std::abs(iPart->pdgId())==14 || std::abs(iPart->pdgId())==16) && iPart->isLastCopy() )
    {
      TLorentzVector thisNuLVec;
      thisNuLVec.SetPtEtaPhiE( iPart->pt(), iPart->eta(), iPart->phi(), iPart->energy() );
      int numid = find_idx( *(iPart->mother(0)) );
      if(std::abs(numid)==5) NuFromBLVec->push_back( thisNuLVec );
      else NuOtherLVec->push_back( thisNuLVec );
    }
  ...
  //2 types of neutrino LVec
  iEvent.put(NuFromBLVec, "NuFromBLVec");
  iEvent.put(NuOtherLVec, "NuOtherLVec");
